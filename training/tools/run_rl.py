...
def parse_args():
    ...
    parser.add_argument('--expert-from', default=None, help='the expert model for warm up')
...
def main_mfrl_brl(cfg, args, rollout, evaluator, logger):
    ...
    cfg_agent = deepcopy(cfg.agent)
    if cfg.get('expert_from', None) is not None: cfg_expert = deepcopy(cfg.expert) #cfg中新加入expert的网络结构
    if cfg.agent.type in MFRL:
        agent = build_mfrl(cfg.agent)
        if cfg.get('expert_from', None) is not None: agent.exp_agent = build_mfrl(cfg_expert) #
    elif cfg.agent.type in BRL:
        agent = build_brl(cfg.agent)
        if cfg.get('expert_from', None) is not None: agent.exp_agent = build_brl(cfg_expert) #
    else:
    ...
    agent.exp = False
    if cfg.get('expert_from', None) is not None:
        load_checkpoint(agent.exp_agent, cfg.expert_from, map_location='cpu')
        agent.exp = True
    ...
    if len(args.gpu_ids) > 1:
        ...
        if cfg.get('expert_from', None) is not None:
            if isinstance(agent.exp_agent,torch.nn.DataParallel):
                agent.exp_agent=agent.exp_agent.module
            agent.exp_agent.eval()

    if not args.evaluation:
        replay = build_replay(cfg.replay_cfg)
        if cfg.get('expert_replay_cfg',None) is not None:
            is_gail = True
            ...
            if cfg.get('tmp_replay_cfg',None) is not None:
                ...
                tmp_replay = None
                is_gail = False
            train_rl(agent, rollout, evaluator, cfg.env_cfg, replay, work_dir=cfg.work_dir, eval_cfg=cfg.eval_cfg, expert_replay=expert_replay, tmp_replay=tmp_replay, is_GAIL=is_gail,
                 **cfg.train_mfrl_cfg)
        else:
...
def main():
    ...
    if args.expert_from is not None:
        cfg.expert_from = args.expert_from
    ...
    if cfg.get('env_cfg', None) is not None:
        ...
        if cfg.get('expert', None) is not None: #
            cfg.expert['obs_shape'] = obs_shape #
            cfg.expert['action_shape'] = action_shape #
            cfg.expert['action_space'] = action_space #
