...
class GAIL(BaseAgent):
    ...
    def update_discriminator(self, expert_replay, tmp_replay):
        ...
        discrim_loss = self.discrim_criterion(expert_out, torch.zeros((expert_out.shape[0],1), device = self.device)) + \
            self.discrim_criterion(tmp_out, torch.ones((expert_out.shape[0],1), device=self.device))
